                ...
                <div class="col-md-6 col-md-offset-1">
                    ...
                    <div class="form-group">
                        <input type="checkbox" class="checkbox" value="1" name="show_customer_dashboard" id="show_customer_dashboard"
                            <?= $this->input->post('show_customer_dashboard') ? 'checked="checked"' : ''; ?>>
                    ...
                    <div class="form-group standard">
                        ...
                        <div class="input-group">
                            <?= form_input('alert_quantity', (isset($_POST['alert_quantity']) ? $_POST['alert_quantity'] : ($product ? $this->sma->formatQuantityDecimal($product->alert_quantity) : '')), 'class="form-control tip" id="alert_quantity"') ?>
...
-->
                    ...
                    <div class="form-group">
                        <input name="cf" type="checkbox" class="checkbox" id="extras" value=""
                            <?= isset($_POST['cf']) ? 'checked="checked"' : '' ?> />
                    ...
                    <div class="row" id="extras-con" style="display: none;">
                        ...
                        <div class="col-md-4">
                            <div class="form-group all">
                                <?= lang('profit_share', 'profit_share') ?>
                        ...
                        <div class="col-md-4">
                            <div class="form-group all">
                                <?= lang('profit_share_marketing', 'profit_share_marketing') ?>
...
$(document).ready(function() {
    ...
    function add_product_item(item) {
        ...
        $.each(items, function() {
            var row_no = this.id;
            var newTr = $('<tr id="row_' + row_no + '" class="item_' + this.id +
                '" data-item-id="' +
                row_no + '"></tr>');
            tr_html = '<td><input name="combo_item_id[]" type="hidden" value="' + this.id +
                ...
                '"><span id="name_' + row_no + '">' + this.code + ' - ' + this.name +
                '</span></td>';
            tr_html +=
    ...
    function calculate_price() {
        ...
        var pp = 0;
        $.each(rows, function() {
            pp += formatDecimal(parseFloat($(this).find('.rprice').val()) * parseFloat($(this).find(
                '.rquantity').val()));
        });
        $('#price').val(pp);
        return true;
    ...
    $('#addSupplier').click(function() {
        if (su <= 5) {
            ...
            var html =
                '<div style="clear:both;height:5px;"></div><div class="row"><div class="col-xs-12"><div class="form-group"><input type="hidden" name="supplier_' +
                su + '", class="form-control" id="supplier_' + su +
                '" placeholder="<?= lang("select") . ' ' . lang("supplier") ?>" style="width:100%;display: block !important;" /></div></div><div class="col-xs-6"><div class="form-group"><input type="text" name="supplier_' +
    ...
    $(document).on('click', '#updateAttr', function() {
        ...
        row.children().eq(1).html('<input type="hidden" name="attr_warehouse[]" value="' + wh +
            '"><input type="hidden" name="attr_wh_name[]" value="' + wh_name + '"><span>' +
            wh_name + '</span>');
        row.children().eq(2).html('<input type="hidden" name="attr_quantity[]" value="' + ($(
            '#aquantity').val() ? $('#aquantity').val() : 0) + '"><span>' + decimalFormat($(
            '#aquantity').val()) + '</span>');
        row.children().eq(3).html('<input type="hidden" name="attr_price[]" value="' + $('#aprice')
            .val() + '"><span>' + currencyFormat($('#aprice').val()) + '</span>');
        $('#aModal').modal('hide');
    });
});
        ...
        $('#' + id).val(v).select2({
            ...
            initSelection: function(element, callback) {
                $.ajax({
                    type: "get",
                    async: false,
                    url: "<?= admin_url('suppliers/getSupplier') ?>/" + $(element)
                        .val(),
                    dataType: "json",
