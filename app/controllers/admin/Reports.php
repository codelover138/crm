...
class Reports extends MY_Controller
{
    ...
    function getSalesReport($pdf = NULL, $xls = NULL)
    {
        ...
        if ($pdf || $xls) {
            ...
            $this->db
                ->select("date, reference_no, warehouses.name, customer, GROUP_CONCAT(CONCAT(" . $this->db->dbprefix('sale_items') . ".product_name, ' (', " . $this->db->dbprefix('sale_items') . ".quantity, ')') SEPARATOR '\n') as iname, grand_total, paid,SUM((". $this->db->dbprefix('sale_items').".real_unit_price * (".$this->db->dbprefix('sale_items').".gst / 100))) as pmo,SUM((". $this->db->dbprefix('sale_items').".real_unit_price * (".$this->db->dbprefix('sale_items').".cgst / 100)))pmm, payment_status", FALSE)
                ->from('sales')
            ...
            if (!empty($data)) {
                ...
                $this->excel->getActiveSheet()->SetCellValue('I1', lang('profit_share'));
                $this->excel->getActiveSheet()->SetCellValue('J1', lang('profit_share_marketing'));
                ...
                foreach ($data as $data_row) {
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('I' . $row, $data_row->pmo);
                    $this->excel->getActiveSheet()->SetCellValue('J' . $row, $data_row->pmm);
               ...
               // $this->excel->getActiveSheet()->getStyle("F" . $row . ":H" . $row)->getBorders()
                ...
                $this->excel->getActiveSheet()->SetCellValue('I' . $row, $pmo);
                $this->excel->getActiveSheet()->SetCellValue('J' . $row, $pmm);
            ...
            $this->load->library('datatables');
            $this->datatables
                ->select("DATE_FORMAT(date, '%Y-%m-%d %T') as date, reference_no, warehouses.name, customer, FSI.item_nane as iname, grand_total, paid, (grand_total-paid) as balance,FSI.pmo,FSI.pmm,  payment_status, {$this->db->dbprefix('sales')}.id as id", FALSE)
                ->from('sales')
