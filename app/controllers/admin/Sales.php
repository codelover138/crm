...
class Sales extends MY_Controller
{
    ...
    public function getSales($warehouse_id = null)
    {
        ...
        if ($warehouse_id) {
            $this->datatables
                ->select("{$this->db->dbprefix('sales')}.id as id, DATE_FORMAT({$this->db->dbprefix('sales')}.date, '%Y-%m-%d %T') as date, reference_no, {$this->db->dbprefix('companies')}.vat_no, {$this->db->dbprefix('sales')}.customer, sale_status, grand_total, paid, (grand_total-paid) as balance, payment_status, {$this->db->dbprefix('sales')}.attachment, return_id")
                ->from('sales')
        ...
        if (!$this->Customer && !$this->Supplier && !$this->Owner && !$this->Admin && $this->session->userdata('view_right') === '2') {
            $this->datatables->where('service_provider', $this->session->userdata('user_id'))->or_where('created_by', $this->session->userdata('user_id'));
        }
    ...
    public function consent($id = null, $view = null, $save_bufffer = null)
    {
        ...
        $name = lang("consent") . "_" . str_replace('/', '_', $inv->reference_no) . ".pdf";
        $html = $this->load->view($this->theme . 'sales/consent', $this->data, true);
        if (! $this->Settings->barcode_img) {
            $html = preg_replace("'\<\?xml(.*)\?\>'", '', $html);
        }

        if ($view) {
            $this->load->view($this->theme . 'sales/consent', $this->data);
        } elseif ($save_bufffer) {
            return $this->sma->generate_pdf($html, $name, $save_bufffer, $this->data['biller']->invoice_footer);
        } else {
            $this->sma->generate_pdf($html, $name, false, $this->data['biller']->invoice_footer);
    ...
    public function consent_sms($id = null, $view = null, $save_bufffer = null)
    {
        $this->sma->checkPermissions();
        ...
        if($this->tec_twilio->send("+393297732293","+393294906396",$message)){
            $this->session->set_flashdata('message', lang("Sms_Sent"));
            admin_redirect("sales");
        }else{
            $this->session->set_flashdata('error', lang("Error_Sending_SMS"));
            admin_redirect("sales");
        }
    ...
    public function add($quote_id = null)
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            for ($r = 0; $r < $i; $r++) {
                ...
                if (isset($item_code) && isset($real_unit_price) && isset($unit_price) && isset($item_quantity)) {
                    ...
                    $product = array(
                        ...
                        'tax' => $tax,
                        'gst'=>$product_details->cf1,
                        'cgst'=>$product_details->cf2,
            ...
            if (isset($_FILES['document']) && count($_FILES['document']['name']) > 0) {
                ...
                // Array to hold all uploaded file names
                $uploadedFiles = [];

                // Loop through each file to handle upload
                foreach ($_FILES['document']['name'] as $key => $fileName) {
                    if ($_FILES['document']['size'][$key] > 0) {
                        ...
                        // Sanitize the customer name to avoid invalid characters in file names
                        $customerSafeName = preg_replace('/[^a-zA-Z0-9_]/', '_', $customer_name->name);

                        // Construct the custom file name: date_customername_phone.extension
                        $customFileName = $originalFileName.'_'.$date . '_' . $customerSafeName . '_' . $customer_name->phone . '.' . $fileExtension;

                        // Upload configuration for each file
                        ...
                        // Initialize the upload library for this file
                ...
                // Concatenate the uploaded file names with commas to save in DB
                if (!empty($uploadedFiles)) {
                    $photo = implode(',', $uploadedFiles);
                    $data['attachment'] = $photo;  // Save into your data array for DB insert
                }
            }
    ...
    public function edit($id = null)
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            for ($r = 0; $r < $i; $r++) {
                ...
                if (isset($item_code) && isset($real_unit_price) && isset($unit_price) && isset($item_quantity)) {
                    $product_details = $item_type != 'manual' ? $this->sales_model->getProductByCode($item_code) : null;
                    $invoice_item=$this->site->getInvoiceProductByProductCodeAndId($id,$item_code);
                    // profit_margin
                    $cf1=($invoice_item->gst ? $invoice_item->gst :$product_details->cf1);
                    $cf2=($invoice_item->cgst ? $invoice_item->cgst :$product_details->cf2);
    ...
    public function update_status($id)
    {
        ...
        if ($this->form_validation->run() == true && $this->sales_model->updateStatus($id, $status, $note,$service_provider,$inv)) {
    ...
    {
        // Get the comma-separated filenames from POST
        $file_names = $this->input->post('attachments');

        // Check if $file_names is not empty
        if (empty($file_names)) {
            $this->session->set_flashdata('error', 'No files to download.');
            redirect($_SERVER["HTTP_REFERER"]);
        }

        // Explode the comma-separated string into an array of file paths
        ...
        $zipFileName = 'files_' . date('Ymd_His') . '.zip'; // Name of the zip file to download
        $zipPath = FCPATH . 'assets/tmp/' . $zipFileName;  // Temporary path to store the zip file

        // Create a zip file
        $zip = new ZipArchive();
        if ($zip->open($zipPath, ZipArchive::CREATE) === TRUE) {
            foreach ($files as $file) {
                $filePath = FCPATH . $file;  // Ensure full path to each file
                if (file_exists($filePath)) {
                    $zip->addFile($filePath, basename($file));  // Add each file to the zip archive
                } else {
                    $this->session->set_flashdata('error', "File $filePath does not exist.");
                }
            }
            $zip->close();  // Close the zip file after adding all files

            // Force download of the zip file if it was successfully created
            if (file_exists($zipPath)) {
                ...
                $this->session->set_flashdata('error', 'ZIP file could not be created.');
                redirect($_SERVER["HTTP_REFERER"]);
            }
        } else {
            $this->session->set_flashdata('error', 'Failed to create ZIP file.');
            redirect($_SERVER["HTTP_REFERER"]);
        }
    }
