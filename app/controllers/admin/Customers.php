...
class Customers extends MY_Controller
{
    ...
    function view($id = NULL)
    {
        ...
        $this->data['marketing_officer'] = $this->site->getUserById($customer->marketing_officer);
    ...
    function add()
    {
        ...
        $this->form_validation->set_rules('email', lang("email_address"), 'is_unique[companies.email]');
         $this->form_validation->set_rules('phone', lang("phone"), 'trim|is_unique[companies.phone]|regex_match[/^\+?[0-9]{10,12}$/]|required');
        ...
        if ($this->form_validation->run('companies/add') == true) {
            ...
            $data = array('name' => $this->input->post('name'),
                ...
                'marketing_officer' => $this->input->post('marketing_officer'),
        ...
        if ($this->form_validation->run() == true && $cid = $this->companies_model->addCompany($data)) {
            ...
            $this->data['marketing_office'] = $this->site->getAllUser();
    ...
    function edit($id = NULL)
    {
        ...
        if ($this->input->post('email') != $company_details->email) {
         ...
         if ($this->input->post('phone') != $company_details->phone) {
            $this->form_validation->set_rules('phone', lang("phone"), 'is_unique[companies.phone]|regex_match[/^\+?[0-9]{10,12}$/]');
        }
    ...
    function add_quick()
    {
        ...
        $this->form_validation->set_rules('email', lang("email_address"), 'trim|required|valid_email|is_unique[companies.email]');
        $this->form_validation->set_rules('phone', lang("phone"), 'trim|required|regex_match[/^\+?[0-9]{10,12}$/]');
        ...
        if ($this->form_validation->run() == true) {
            ...
            if ($cid = $this->companies_model->addCompany($data)) {
                $customer = $this->companies_model->getCompanyByID($cid);
                $customer_name = trim($customer->name . ' ' . $customer->last_name) . ' (' . $customer->phone . ')';
                ...
                $this->output->set_output(json_encode(array('error' => lang('customer_add_failed'))));
                return;
            }
        } else {
    ...
    function getCustomer($id = NULL)
    {
        ...
        $this->sma->send_json(array(array('id' => $row->id, 'text' => ($row->name != '' ? $row->name.' '.$row->last_name : ''))));
    }
