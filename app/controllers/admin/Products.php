...
class Products extends MY_Controller
{
    ...
    function getProducts($warehouse_id = NULL)
    {
        ...
        <ul class="dropdown-menu pull-right" role="menu">
            <li><a href="' . admin_url('products/view/$1') . '"><i class="fa fa-file-text-o"></i> ' . lang('product_details') . '</a></li>
        ...
        $this->load->library('datatables');
            ...
            $this->datatables
                ->select($this->db->dbprefix('products') . ".id as productid, {$this->db->dbprefix('products')}.code as code, {$this->db->dbprefix('products')}.name as name, {$this->db->dbprefix('brands')}.name as brand, {$this->db->dbprefix('categories')}.name as cname,price as price", FALSE)
                ->from('products')
    ...
    function add($id = NULL)
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $data = array(
                ...
                'featured' => $this->input->post('show_customer_dashboard') ? 1 : 0,
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (!empty($_POST['val'])) {
                if ($this->input->post('form_action') == 'sync_quantity') {
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('D1', lang('category_code'));
                    $this->excel->getActiveSheet()->SetCellValue('E1', lang('price'));
                    $this->excel->getActiveSheet()->SetCellValue('F1', lang('tax_rate'));
                    $this->excel->getActiveSheet()->SetCellValue('G1', lang('tax_method'));
                    $this->excel->getActiveSheet()->SetCellValue('H1',  lang('subcategory_code'));
                    $this->excel->getActiveSheet()->SetCellValue('I1', lang('pcf1'));
                    $this->excel->getActiveSheet()->SetCellValue('J1', lang('pcf2'));
                    $this->excel->getActiveSheet()->SetCellValue('K1', lang('pcf3'));
                    $this->excel->getActiveSheet()->SetCellValue('L1', lang('pcf4'));
                    $this->excel->getActiveSheet()->SetCellValue('M1', lang('pcf5'));
                    $this->excel->getActiveSheet()->SetCellValue('N1', lang('pcf6'));
                    ...
                    foreach ($_POST['val'] as $id) {
                        ...
                        $this->excel->getActiveSheet()->SetCellValue('F' . $row, $product->tax_rate_name);
                        $this->excel->getActiveSheet()->SetCellValue('G' . $row, $product->tax_method ? lang('exclusive') : lang('inclusive'));
                        $this->excel->getActiveSheet()->SetCellValue('H' . $row, $product->subcategory_code);
                        $this->excel->getActiveSheet()->SetCellValue('I' . $row, $product->cf1);
                        $this->excel->getActiveSheet()->SetCellValue('J' . $row, $product->cf2);
                        $this->excel->getActiveSheet()->SetCellValue('K' . $row, $product->cf3);
                        $this->excel->getActiveSheet()->SetCellValue('L' . $row, $product->cf4);
                        $this->excel->getActiveSheet()->SetCellValue('M' . $row, $product->cf5);
                        $this->excel->getActiveSheet()->SetCellValue('N' . $row, $product->cf6);
                        $row++;
                    }
