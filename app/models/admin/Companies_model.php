...
class Companies_model extends CI_Model
{
    ...
    public function getCustomerSuggestions($term, $limit = 10)
    {
        ...
        $this->db->select("id, (CASE WHEN company = '-))' THEN name ELSE  CONCAT(name) END) as text, (CASE WHEN company = '-' THEN name ELSE CONCAT(company, ' (', name, ')') END) as value, phone", FALSE);
        $this->db->where(" (id LIKE '%" . $term . "%' OR name LIKE '%" . $term . "%' OR vat_no LIKE '%" . $term . "%' OR email LIKE '%". $term . "%' OR email LIKE '%" . $term . "%' OR phone LIKE '%" . $term . "%') ");
